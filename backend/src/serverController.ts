import {
  ...
  simkolClient,
  simrailClient,
  strictAwsSimrailClient,
} from "./simrailClient.js";
import {
  BASE_AWS_API,
  BASE_SIMRAIL_API,
  POSTS,
...
export async function getServerCodeList() {
  ...
  return (response.data as ApiResponse<Server>).data?.map(
    (server) => server.ServerCode,
  );
}

export function getServerList(req: express.Request, res: express.Response) {
  return simrailClient
    .get("servers-open", BASE_SIMRAIL_API)
    ?.then((e) => {
      const serverData = (e.data as ApiResponse<Server>).data;
      // Sort by Language code so the client doesn't have to
      serverData.sort((s1, s2) =>
        s1.ServerCode.slice(0, 2).toUpperCase() <
        s2.ServerCode.slice(0, 2).toUpperCase()
          ? -1
          : 0,
      );
      ...
      serverData.sort((s1, s2) =>
        s1.ServerCode.slice(0, 2).toUpperCase() ===
          s2.ServerCode.slice(0, 2).toUpperCase() &&
        s1.ServerCode.slice(2) < s2.ServerCode.slice(2)
          ? -1
          : 0,
      );
      return res
...
export function getStationsList(req: express.Request, res: express.Response) {
  return simrailClient
    .get("stations-open?serverCode=" + req.params.serverCode)
    ?.then((e) => {
      return res
...
export function getTrainsList(req: express.Request, res: express.Response) {
  return simrailClient
    .get(`trains-open?serverCode=${req.params.serverCode}`)
    ?.then((e) => {
      return res
        .setHeader(
          "Cache-control",
          "public, max-age=10, must-revalidate, stale-if-error=30",
        )
...
export async function getTrainsListForPost(
  req: express.Request,
  res: express.Response,
  trainTimetables: IEdrServerTrain[],
) {
  try {
      ...
      .send(
        await Promise.all(
          trainList.map(async (train) => {
            ...
            const trainTimetable = trainTimetables.find(
              (timetable) => timetable.trainNoLocal === train.TrainNoLocal,
            );
            if (trainTimetable !== undefined) {
              ...
              const postsInTimetable = trainTimetable.timetable.filter(
                (checkpoint) =>
                  internalIds.includes(parseInt(checkpoint.pointId)),
              );
              const hasTrainLeftThePost =
                postsInTimetable !== undefined && postsInTimetable.length > 0
                  ? train?.TrainData.VDDelayedTimetableIndex >
                    postsInTimetable[postsInTimetable.length - 1].indexOfPoint
                  : true;
              if (hasTrainLeftThePost) {
                ...
                return {
                  ...train,
                  distanceFromStation:
                    osrmResult?.routes?.[0]?.distance !== undefined
                      ? Math.round(osrmResult.routes[0].distance / 10) / 100
                      : 0,
                };
              }
            } else {
...
export function getServerTz(req: express.Request, res: express.Response) {
  return simrailClient
    .get(`getTimeZone?serverCode=${req.params["serverCode"]}`, BASE_AWS_API)
    ?.then((e) => {
      return res
...
export function getServerTime(req: express.Request, res: express.Response) {
  ...
  if (cachedTimeData === undefined || cachedTimeData[0] >= 120) {
    return simrailClient
      .get(`gettime?serverCode=${req.params["serverCode"]}`, BASE_AWS_API)
      ?.then((e) => {
...
export function getPlayer(req: express.Request, res: express.Response) {
  ...
  return axios
    .get(
      `https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/?key=${STEAM_API_KEY}&format=json&steamids=${req.params.steamId}`,
    )
    .then((response) => {
...
export function getFullTimetable(serverCode: string) {
  return strictAwsSimrailClient.get(
    "getAllTimetables?serverCode=" + serverCode,
  );
}
...
export function getEdrTimetable(serverCode: string) {
  return strictAwsSimrailClient.get(
    "getEDRTimetables?serverCode=" + serverCode,
  );
}
...
export function getOsrmDataFromSelfApi(
  startLon: number,
  startLat: number,
  endLon: number,
  endLat: number,
  ...
  return osrmClient.get(
    `route/v1/train/${Math.round(startLon * 100000) / 100000},${Math.round(startLat * 100000) / 100000};${Math.round(endLon * 100000) / 100000},${Math.round(endLat * 100000) / 100000}?overview=false&continue_straight=true`,
  );
}
