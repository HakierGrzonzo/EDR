...
export const getStationTimetable = async (
  ...
  const trainsForStation = trainList.filter((train) =>
    train.timetable.some(
      (checkpoint) => parseInt(checkpoint.pointId) === stationId,
    ),
  );
  const withDynamicData: Promise<IFrontendStationTrainRow>[] =
    trainsForStation.map(async (train) => {
      const stationEntry = train.timetable.find(
        (checkpoint) => parseInt(checkpoint.pointId) === stationId,
      );
      ...
      const stationIndex = train.timetable.findIndex(
        (checkpoint) => parseInt(checkpoint.pointId) === stationId,
      );
      ...
      return {
        ...
        actualArrivalObject:
          stationEntry.actualArrivalTime != null
            ? new Date(stationEntry.actualArrivalTime)
            : new Date(0),
        actualDepartureObject:
          stationEntry.actualDepartureTime != null
            ? new Date(stationEntry.actualDepartureTime)
            : new Date(3000, 12, 31),
        scheduledArrivalObject:
          stationEntry.arrivalTime != null
            ? new Date(stationEntry.arrivalTime)
            : new Date(0),
        scheduledDepartureObject:
          stationEntry.departureTime != null
            ? new Date(stationEntry.departureTime)
            : new Date(3000, 12, 31),
  ...
  return Promise.all(withDynamicData).then((result) =>
    _.sortBy(result, "scheduledArrivalObject"),
  );
};
