...
const mergePostRows = (allPostsResponse: IFrontendStationTrainRow[][]) => {
  ...
  const keyedSecondaryPostsRows = secondaryPostsRows.map((secondaryPostRows) =>
    _.keyBy(secondaryPostRows, "trainNoLocal"),
  );

  // Handle stations that have multiple posts, merge their data into a single entry
  const mergedPostsRows = primaryPostRows.reduce(
    (acc, v) => [
      ...acc,
      {
        ...v,
        secondaryPostsRows: keyedSecondaryPostsRows
          .map((kspr) => kspr[v.trainNoLocal])
          .filter(
            (row): row is Exclude<typeof row, undefined> => row !== undefined,
          ),
      },
    ],
...
export async function dispatchController(
  ...
  try {
    ...
    const data = await Promise.all(
      postsToFetch.map((post) => getStationTimetable(post, trainList)),
    );
    const mergedPosts = mergePostRows(data);
    return res
      .setHeader(
        "Cache-control",
        "public, max-age=28800 stale-if-error=604800 must-revalidate",
      )
      .send(mergedPosts);
  } catch (e) {
...
export async function trainTimetableController(
  req: express.Request,
  res: express.Response,
  trainList: IEdrServerTrain[],
  ...
  if (trainList === undefined || trainList === null) {
    console.error(
      "Timetable is empty, cannot process train timetable request!",
    );
    return res.sendStatus(500);
  }
